...
def main():
    ...
    assert previous["status"] == "completed" and all(
        r["acceptance_passed"] for r in result["rows"]
    )
    ...
    try:
        ...
        eligible = sorted(
            p.name[5:]
            for p in shared.RAW.glob("date=*")
            if re.fullmatch(r"date=\d{8}", p.name)
            and p.name[5:] not in excluded
        ...
        assert shared.digest(binary) == previous["binary_sha256"], (
            "Expected unchanged validated binary"
        )
        hashes = {}
        for path in sorted(
            [
