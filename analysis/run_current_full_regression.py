...
def previously_tested_dates():
    ...
    for path in (ROOT / "reports").glob("*/manifest.json"):
        ...
        for key in (
            ...
            "random_days",
            "days",
            "previously_tested_dates",
            "excluded_dates",
        ):
            dates.update(
                str(day)
                for day in manifest.get(key, [])
                if re.fullmatch(r"20\d{6}", str(day))
            )
...
def validate_report(report, day, market, sources):
    ...
    assert report["replay"]["input_rows"] == sum(
        s["rows"] for s in sources if s["date"] == day and s["feed"] in feeds
    )
    ...
    return (
        all(report[k] == 0 for k in ("mismatched", "data_errors", "missing_source"))
        and report["comparable_anchors"] > 0
    )
...
def run_job(binary, day, market, stage, sources):
    ...
    cmd = [
        ...
        str(timings_path),
        "--temp-root",
        str(ROOT / "target/current-regression-spool"),
        "--max-detail-records",
        "0",
    ]
    receipt = {
    ...
    try:
        with (OUT / f"{name}.log").open("x") as log:
            proc = subprocess.run(
                ["/usr/bin/time", "-v", "-o", str(OUT / f"{name}.time.txt")] + cmd,
                cwd=ROOT,
                stdout=log,
        ...
        if not report_path.exists():
            receipt.update(
                status="replay_aborted",
                error=(OUT / f"{name}.log").read_text()[-16000:],
            )
        else:
            ...
            for source in sources:
                if source["date"] == day:
                    ...
                    assert (stat.st_size, stat.st_mtime_ns) == (
                        source["size"],
                        source["mtime_ns"],
                    )
            ...
            assert (
                abs(
                    t["profiled_total_seconds"]
                    - t["restore_total_seconds"]
                    - t["validation_total_seconds"]
                    - t["unattributed_seconds"]
                )
                < 1e-5
            )
            receipt.update(
            ...
            receipt["first_anomalies"] = [
                r
                for r in report["records"]
                if r["outcome"] not in ("matched", "excluded_by_status")
            ][:20]
            receipt["throughput_applied_events_per_restore_second"] = (
                report["replay"]["applied_events"] / t["restore_total_seconds"]
            )
    except Exception:  # noqa: BLE001 -- Persist each worker failure in its receipt; the campaign gate fails.
        receipt.update(
            status="driver_error",
...
def summarize(manifest):
    ...
    for day in manifest["baseline_days"] + manifest.get("random_days", []):
        for market in ("SH", "SZ"):
            ...
            rows.append(
                ...
                if path.exists()
                else {"date": day, "market": market, "status": "queued"}
            )
    summary = {
        "updated_at": utc(),
        "campaign_status": manifest["status"],
        "random_stage": manifest["random_stage"],
        "baseline_gate_passed": manifest.get("baseline_gate_passed", False),
        "all_jobs_finished": all(
            r["status"] in ("completed", "replay_aborted", "driver_error") for r in rows
        ),
        "all_scheduled_jobs_passed": bool(rows)
        and all(r.get("acceptance_passed", False) for r in rows),
        "rows": rows,
    }
    save(OUT / "summary.json", summary)
    lines = [
        "# 当前代码沪深全市场回归与 profiling",
        "",
    ...
    for row in rows:
        ...
        values = [
            ...
            c.get("matched", "—"),
            c.get("mismatched", "—"),
            c.get("excluded_by_status", "—"),
            f"{c.get('data_errors', '—')}/{c.get('missing_source', '—')}",
            fmt(t.get("restore_total_seconds")),
            fmt(t.get("validation_total_seconds")),
            fmt(row.get("elapsed_seconds")),
            fmt(row["peak_rss_kib"] / 1048576 if row.get("peak_rss_kib") else None),
        ]
        lines.append("| " + " | ".join(map(str, values)) + " |")
    lines += [
        "",
        "恢复 = 输入读取/分片 + 排除验证回调的回放 + 分片清理；对比 = 参考读取/分类 + 比较回调 + 报告整理。",
...
def stage(manifest, binary, days, name):
    with ThreadPoolExecutor(max_workers=WORKERS) as pool:
        pending = {
            pool.submit(run_job, binary, day, market, name, manifest["sources"]): (
                day,
                market,
            )
            for day in days
...
def main():
    ...
    try:
        ...
        eligible = sorted(
            p.name[5:]
            for p in RAW.glob("date=*")
            if re.fullmatch(r"date=\d{8}", p.name)
            and p.name[5:] not in historical
        ...
        manifest = {
            ...
            "baseline_days": DAYS,
            "random_days": [],
            "random_stage": "waiting_for_baseline",
            "seed": SEED,
            "random_count": 3,
            "eligible_random_dates": eligible,
            "previously_tested_dates": sorted(historical),
            "max_workers": WORKERS,
            "features": ["profiling"],
            "scope": "all_stocks_and_etfs",
            "frozen_binary": str(frozen),
            "binary_sha256": digest(frozen),
            "sources": inventory(DAYS),
        ...
        if not passed:
            manifest.update(
                ...
                random_stage="not_started_baseline_failed",
                finished_at=utc(),
            )
        elif len(eligible) < 3:
            manifest.update(
                status="random_selection_blocked",
                random_stage="insufficient_untested_dates",
                finished_at=utc(),
            )
        else:
            days = sorted(random.Random(SEED).sample(eligible, 3))
            manifest.update(
                random_days=days, random_stage="running", status="random_running"
            )
            manifest["sources"].extend(inventory(days))
            save(OUT / "manifest.json", manifest)
            passed = stage(manifest, frozen, days, "random")
            manifest.update(
                status="completed" if passed else "random_failed",
                random_stage="completed",
                finished_at=utc(),
            )
        save(OUT / "manifest.json", manifest)
        summarize(manifest)
        ...
        if manifest is None:
            manifest = {
                "baseline_days": DAYS,
                "random_days": [],
                "random_stage": "not_started",
            }
        manifest.update(
            status="driver_error", error=traceback.format_exc(), finished_at=utc()
        )
        save(OUT / "manifest.json", manifest)
        summarize(manifest)
        raise
...
if __name__ == "__main__":
    ...
    if args.launch:
        ...
        print(
            json.dumps(
                {"driver_pid": child.pid, "log": str(log_path), "output": str(OUT)}
            )
        )
    elif args.summarize:
