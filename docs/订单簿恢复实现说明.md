...
| [validation.rs](../src/production/validation.rs) | 公开入口、共享状态、观察器协调：加载参考 → 回放观察 → 候选比较 → 生成报告 |
| [loader.rs](../src/production/validation/loader.rs)、[columns.rs](../src/production/validation/columns.rs)、[phases.rs](../src/production/validation/phases.rs) | 参考文件加载、Arrow 解码、选帧与审计、重复时间检查 |
| [candidate.rs](../src/production/validation/candidate.rs)、[reference.rs](../src/production/validation/reference.rs) | 候选窗口与缓存、快速比较、按需生成详细差异 |
| [sz_close_price.rs](../src/production/validation/sz_close_price.rs)、[close_range.rs](../src/production/validation/close_range.rs) | 前者追踪成交并协调深市收盘价；后者负责收盘价格范围投影，均不修改恢复盘口 |
| [report.rs](../src/production/validation/report.rs) | 单帧结果定性、累计计数、覆盖统计与 v2 报告组装 |

大型私有实现测试位于各模块对应的 `tests.rs`，仍通过 `#[cfg(test)] mod tests`
运行，不为测试扩大生产接口可见性。
...
阅读 pending 路径时，先看 `replay.rs` 的 `process_sz_channel` 双流归并，再看
`collect_pending_group` 和 `apply_pending_group`。前者只消费有界响应组并调用
`sz_pending.rs` 校验、分类，不修改盘口；后者按源顺序应用订单和响应，完成余量入簿后
再通知成交审计与组外观察器。应用不是整组事务，失败时保留此前已成功应用的副作用；
组内中间盘口不会对外发布，前瞻游标停留在原有边界。
...
生产截面使用严格 `quote_time < T`；验证候选窗与其独立。以下时间检查只针对委托、撤单、
成交业务事件：沪市 `S` 状态记录按 `BizIndex` 处理，但不推进业务时间、普通验证窗口或
定时截面。`CCALL` 后集中发布的委托可以携带比状态记录更早的原始 `TickTime`。
开启截面输出时拒绝业务时间回退；未输出截面时，当前实现允许并审计同一秒内回退，
跨秒回退仍失败，不改变原生顺序。
审计计数为 `same_second_quote_time_regressions`；这种容忍不用于固定间隔截面输出。

沪市收盘在逐笔 CLOSE 处捕获；深市完整耗尽通道两流后捕获，不按 15:00 截断。
沪市收盘回调立即执行；开启输出时，每标的暂存一个已捕获的收盘截面，通道 EOF 补齐
普通截面后写出。逐笔 CLOSE 后的业务记录报错，不修改收盘检查点；重复状态不重复捕获。
深市边界为 `max(当日15:00, 已应用行情时间上界)`；15:00 后事件计入
`sz_after_close_events(_by_symbol)`，未经阶段确认阻断 E0 验收。

validation 单独加载 raw snapshot，按阶段资格选择真实参考帧，通过 StateObserver 接收
恢复状态；未选中原因独立汇总，额外静态帧不解析盘口，必要原始顺序检查仍保留。
深市收盘 LastPrice 转换及无涨跌幅限制价格范围投影只作用于比较视图，
