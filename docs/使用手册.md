...
选帧与比较要求只在[验收规范](截面数据验证匹配规则.md)定义；具体运行结果仅在本地 `reports/` 留存。
...
这里的事件指委托、撤单和成交。沪市 `S` 状态时间不推进业务时间检查、定时截面或
普通验证窗口：集合竞价委托可能在状态事件之后按 `BizIndex` 发布，却携带更早的
`TickTime`。恢复器等待后续业务事件或输入耗尽后补齐截面，不按时间重排或改写逐笔。
集合竞价数据延后发布时，完整截面也可能延后输出，不能将其误解为墙钟时刻即时可得。
...
深市仅对正常 `O0→B0`、T0 和 `C0→E0` 选择比较帧。`S0→H0→E0` 只进入选帧审计，
不要求构造空订单簿，不计为匹配，也不因缺少正常锚点而失败；复牌后的正常阶段继续
比较。缺少 C0 或没有有效收盘参考不生成收盘比较；已选参考的逐笔来源缺失、字段非法
或确实不匹配仍失败，输入文件与必要顺序检查也不放宽。
...
| `--max-detail-records N` | 限制失败明细，不限制总计数；CLI 省略时不限 |
...
- 新 JSON 使用 `report_schema_version: 2`。Rust 接口拒绝旧版和未知版本；汇总工具把无
  版本旧报告视为 v1，并与 v2 分组统计，不修改历史文件。
- `records` 仅包含真实选中帧，结果为 Matched、Mismatched、DataError、MissingSource。
  未选中帧不生成 ExcludedByStatus，不再使用 total_anchors、not_comparable 等旧字段。
- `selected_references = matched + mismatched + data_errors + missing_source`；
  `comparable_references = matched + mismatched`。匹配率分母为零时为 null，CLI 显示 N/A。
- `breakdown` 按股票／ETF和阶段汇总实际比较，计数不受明细截断影响。
  默认省略成功明细，`omitted_matched_records/omitted_failure_records` 分别记录省略数量。
- `reference_time_ms` 为必需 epoch 毫秒；`reference_source_row_no` 定位原始参考帧。
  命中明细保留 `matched_candidate_time_ms/raw_sequence/apply_sequence` 和通道；
  延续状态的候选时间可晚于最后事件，空簿序号可为空；失败明细保留最接近候选及差异。
- `selection_audit` 每证券保存 reference_records、status_counts、selected_counts、
  skipped_counts、first_samples 和 coverage。coverage 区分 no_reference_records、
  no_eligible_references、selected；异常原因只保留首个样例，静态重复帧不解析盘口。
- 报告顶层 `coverage` 包含 symbols、symbols_with_selected_references、
  symbols_without_reference_records、symbols_without_eligible_references；
  `by_stage` 按市场、品类、阶段保存 symbols、covered_symbols、selected_references。
  无原始参考和有参考但无有效帧为互斥分类，不重复计数。
- `run_outcome` 为 passed、no_eligible_references、failed。前两者退出 0，失败退出非零；
  零比较不算验收通过，部分覆盖必须同时看 coverage，不能把 Passed 当成全市场覆盖。
- `match_tags` 记录特殊收盘价规则；`close_price_band` 记录 E0 比较投影；
  `continuous_lookahead_ms_by_symbol` 是逐证券实际窗口。
- 接收字段双空时，仅兼容已知的成交额 12 位有效数字舍入及深市上限哨兵变体，
  其他字段仍正常比较；具体门槛见[验证规则](截面数据验证匹配规则.md#接收信息双空的已知数值兼容)。
  `match_tags` 同时累计 `MISSING_RECEPTION_*` 兼容标签；成功明细中的
  `turnover_precision` 和 `close_price_band.upper_limit_normalization` 保留数值审计。
  默认裁剪成功明细不影响标签计数；需要逐帧审计时加 `--retain-matched-records`。
...
- `is_success()` 表示运行无阻断失败（包括零有效参考）；`is_standard_acceptance()` 还要求
  有真实比较、当前
...
性能测量命令与时间拆分见 [Profiling](#profiling)。
...
完整生产测量使用 [Profiling](#profiling) 中的验证 benchmark。
...
比较前后版本时应固定输入、硬件、构建参数、并发数及报告选项，记录缓存和系统负载情况，
使用重复测量区分改进与波动，并核对语义报告及处理计数一致。
各批次将代码／输入指纹、覆盖范围、失败与未选中计数、计时环境与结果保存到本地 `reports/`；
该目录由 Git 忽略，不随克隆提供。耗时不作为跨机器的性能承诺。
