...
原 C++ 订单簿基础迁移已基本完成，本文退出当前开发方案与验收规范体系，不再追加生产
功能或同步接口变更。正文保留迁移时的行为基线、字段映射、有意修复、实施阶段及后续
...
历史代码可从提交 `f3f77c0`（报告历史整理前 `73adb97`）追溯，不应重新引入已退役接口。
